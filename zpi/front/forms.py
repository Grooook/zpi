...
class UserApplicationForm(forms.Form):
    CURRENT_DATE = forms.DateField(required=True, initial=datetime.today(
    ...
    STUDY_DEGREE = forms.ChoiceField(
        required=True, choices=((1, "I"), (2, "II")))
    ...
    STATIONARY = forms.CharField(
        required=True, widget=forms.TextInput(attrs={'autoComplete': "off"}))
    OPINION = forms.CharField(required=True, widget=forms.Textarea(
        attrs={"rows": "4", 'autoComplete': "off"}))
    JUSTIFICATION = forms.CharField(required=True, widget=forms.Textarea(
        attrs={"rows": "4", 'autoComplete': "off"}))
